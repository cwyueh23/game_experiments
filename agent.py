        ...
        for t in range(self.horizon):
            for s in range(self.state_num):
                ...
                if not self.is_minimizer:
                    ...
                    pi1_opt /= pi1_opt.sum()
                    self.strategy[self.horizon-t-1,s]=pi1_opt
                else:
                    # Minimizing player
                    ...
                    pi2_opt /= pi2_opt.sum()
                    self.strategy[self.horizon-t-1,s]=pi2_opt
...
class Agent_eq(Agent):
    def __init__(self, game, horizon, is_minimizer=False,seed=612):
        # Initialize prior as Dirichlet distributions (initially uniform) for transition probabilities
...
class Agent_ps_map(Agent):
    def __init__(self, game, horizon, is_minimizer=False,larger_prior=False,seed=503):
        super().__init__(game, horizon, is_minimizer,seed)
        # Initialize prior as Dirichlet distributions (initially uniform) for transition probabilities
        if larger_prior:
...
class Agent_fp(Agent):
    def __init__(self, game, horizon, is_minimizer=False,larger_prior=False,seed=77):
        super().__init__(game, horizon, is_minimizer,seed)
        if is_minimizer:
    ...
    def solve_value(self):
        for t in range(self.horizon):  # Run value iteration
            for s in range(self.state_num):
                if self.is_minimizer:
                    self.V[self.horizon-t-1,s] = min(np.dot(self.R[s, :,a2] + np.dot(self.T[s,:, a2], self.V[self.horizon-t]),self.opponent_strategy[self.horizon-t-1][s]) for a2 in range(self.action_2_num))
                    action=np.argmin([np.dot(self.R[s, :,a2] + np.dot(self.T[s,:, a2], self.V[self.horizon-t]),self.opponent_strategy[self.horizon-t-1][s]) for a2 in range(self.action_2_num)])
                    self.strategy[self.horizon-t-1,s]=np.zeros(self.action_2_num)
                    self.strategy[self.horizon-t-1,s,action]=1
                else:
                    self.V[self.horizon-t-1,s] = max(np.dot(self.R[s, a1] + np.dot(self.T[s, a1], self.V[self.horizon-t]),self.opponent_strategy[self.horizon-t-1][s]) for a1 in range(self.action_1_num))
                    action=np.argmax([np.dot(self.R[s, a1] + np.dot(self.T[s, a1], self.V[self.horizon-t]),self.opponent_strategy[self.horizon-t-1][s]) for a1 in range(self.action_1_num)])
...
class Agent_fp_map(Agent):
    def __init__(self, game, horizon, is_minimizer=False,larger_prior=False,seed=1985):
        super().__init__(game, horizon, is_minimizer,seed)
        if is_minimizer:
            self.opponent_strategy=[ np.ones((self.state_num,self.action_1_num))/self.action_1_num for i in range(horizon) ]
...
class Agent_fp_true(Agent):
    def __init__(self, game, horizon, is_minimizer=False,larger_prior=False,seed=3):
        super().__init__(game, horizon, is_minimizer,seed)
        if is_minimizer:
            self.opponent_strategy=[ np.ones((self.state_num,self.action_1_num))/self.action_1_num for i in range(horizon) ]
            #self.V=utility.solve_value_by_strategy(self.T,game, horizon,maximal=False)
...
class Agent_sample_ps(Agent):
    def __init__(self, game, horizon, is_minimizer=False,larger_prior=False,seed=68):
        super().__init__(game, horizon, is_minimizer,seed)
        if is_minimizer:
            self.opponent_strategy=np.array([ np.ones((self.state_num,self.action_1_num))/self.action_1_num for i in range(horizon) ])
            self.strategy=np.zeros((horizon,self.state_num,self.action_2_num))
        else:
            self.opponent_strategy=np.array([ np.ones((self.state_num,self.action_2_num))/self.action_2_num for i in range(horizon) ])
            self.strategy=np.zeros((horizon,self.state_num,self.action_1_num))
        self.R=game.R
        self.V=np.zeros((self.horizon+1,self.state_num))
        self.opponent_V=np.zeros((self.horizon+1,self.state_num))
        self.T = np.zeros((self.state_num, game.action_1_num, game.action_2_num, self.state_num))  # Transition probabilities (initialized to zero)
        self.opponent_T=np.zeros((self.state_num, game.action_1_num, game.action_2_num, self.state_num))  # Transition probabilities (initialized to zero)
        self.larger_prior=larger_prior
        if larger_prior:
            self.prior = np.ones((game.state_num_1*game.state_num_2, game.action_1_num*game.action_2_num, game.state_num_1*game.state_num_2))  # Dirichlet params
        else:
    ...
    def update_T(self,T_1,T_2, T_opponent_1, T_opponent_2):
        for s in range(self.state_num):
          for a_1 in range(self.action_1_num):
              for a_2 in range(self.action_2_num):
                for s_next in range(self.state_num):
                  self.T[s,a_1,a_2,s_next] = T_1[s%9,a_1,s_next%9]*T_2[s//9,a_2,s_next//9]
                  self.opponent_T[s,a_1,a_2,s_next] = T_opponent_1[s%9,a_1,s_next%9]*T_opponent_2[s//9,a_2,s_next//9]
        ...
        if self.larger_prior:
          for s in range(self.state_num):
            for a1 in range(self.action_1_num):
                for a2 in range(self.action_2_num):
                    self.T[s, a1,a2] = self.rng.dirichlet(self.prior[s, a1,a2])
                    self.opponent_T[s, a1,a2]= self.rng.dirichlet(self.prior[s, a1,a2])
                    
        else:
            T_1=np.zeros((self.state_num_1,self.action_1_num,self.state_num_1))
            T_2=np.zeros((self.state_num_2,self.action_2_num,self.state_num_2))
            T_opponent_1=np.zeros((self.state_num_1,self.action_1_num,self.state_num_1))
            T_opponent_2=np.zeros((self.state_num_2,self.action_2_num,self.state_num_2))
            # Sample transition probabilities from the Dirichlet prior for each state-action pair
            for s in range(self.state_num_1):
                for a1 in range(self.action_1_num):
                    T_1[s, a1] = self.rng.dirichlet(self.prior_1[s, a1])
                    T_opponent_1[s, a1] = self.rng.dirichlet(self.prior_1[s, a1])
            for s in range(self.state_num_2):
                for a2 in range(self.action_2_num):
                    T_2[s, a2] = self.rng.dirichlet(self.prior_2[s, a2])
                    T_opponent_2[s, a2] = self.rng.dirichlet(self.prior_2[s, a2])
            self.update_T(T_1,T_2, T_opponent_1, T_opponent_2)
    ...
    def solve_value(self):
        # compute the value function and mixed strategies of opponent
        for t in range(self.horizon):
            for s in range(self.state_num):
                # Initialize matrix Q(s) for mixed strategy evaluation
                Q_s = np.zeros((self.action_1_num, self.action_2_num))
                ...
                for a1 in range(self.action_1_num):
                    for a2 in range(self.action_2_num):
                        Q_s[a1, a2] = self.R[s, a1, a2] + np.dot(self.opponent_T[s, a1, a2], self.opponent_V[self.horizon - t])
                ...
                if not self.is_minimizer:
                    ...
                    pi1_opt = np.maximum(pi1_opt, 0)  # Ensure all values are non-negative
                    pi1_opt /= pi1_opt.sum()
                    self.opponent_strategy[self.horizon-t-1,s]=pi1_opt
                else:
                    # Minimizing player
                    objective = cp.Minimize(u)
                    prob = cp.Problem(objective, constraints_min)
                    ...
                    pi2_opt = np.maximum(pi2_opt, 0)  # Ensure all values are non-negative
                    pi2_opt /= pi2_opt.sum()
                    self.opponent_strategy[self.horizon-t-1,s]=pi2_opt

                # Store the optimal value for this timestep and state
                self.opponent_V[self.horizon - t - 1, s] = prob.value
                
        # Solve itself's value function
        for t in range(self.horizon):  
            for s in range(self.state_num):
                if self.is_minimizer:
                    self.V[self.horizon-t-1,s] = min(np.dot(self.R[s, :,a2] + np.dot(self.T[s,:, a2], self.V[self.horizon-t]),self.opponent_strategy[self.horizon-t-1][s]) for a2 in range(self.action_2_num))
...
class Agent_sample_map(Agent):
    def __init__(self, game, horizon, is_minimizer=False,larger_prior=False,seed=9532):
        super().__init__(game, horizon, is_minimizer,seed)
        if is_minimizer:
            self.opponent_strategy=np.array([ np.ones((self.state_num,self.action_1_num))/self.action_1_num for i in range(horizon) ])
            self.strategy=np.zeros((horizon,self.state_num,self.action_2_num))
...
class Agent_sample_true(Agent):
    def __init__(self, game, horizon, is_minimizer=False,larger_prior=False,seed=6007):
        super().__init__(game, horizon, is_minimizer,seed)
        self.T = game.T # True Transition probabilities 
        if is_minimizer:
            self.opponent_strategy=np.array([ np.ones((self.state_num,self.action_1_num))/self.action_1_num for i in range(horizon) ])
            self.strategy=np.zeros((horizon,self.state_num,self.action_2_num))
        else:
    ...
    def update_T(self, T_opponent_1, T_opponent_2):
        for s in range(self.state_num):
          for a_1 in range(self.action_1_num):
              for a_2 in range(self.action_2_num):
                for s_next in range(self.state_num):
        ...
        if self.larger_prior:
          for s in range(self.state_num):
            ...
            for s in range(self.state_num_2):
                for a2 in range(self.action_2_num):
                    T_opponent_2[s, a2] = self.rng.dirichlet(self.prior_2[s, a2])
            self.update_T( T_opponent_1, T_opponent_2)
